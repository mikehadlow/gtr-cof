...
function updateFretboard(tuningInfo: events.TuningChangedEvent): void {
    ...
    const fretData: number[] = getFretData(numberOfFrets)
    ...
    fretboardElement = gtr.node() as SVGGElement
...
function allNotesFrom(index: number, numberOfNotes: number): StringNote[] {
...
function getFretData(fretNum: number): number[] {
    const data: number[] = []
