...
export function init() {
    try {
        const cookieData = cookies.readCookie()

        if (cookieData.hasCookie) {
            const cookieModes = currentScaleFamily.modes.filter(x => x.index === cookieData.modeIndex)
    ...
    const tempChordIndex = currentChordIndex
