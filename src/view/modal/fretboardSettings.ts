import { createModal, createSection, createCheckbox } from ".";
...
export function showFretboardSettings(state: State, raise: (msg: Msg) => void) {
    ...
    createCheckbox(
        ...
        "Left Handed",
        state.isLeftHanded,
        (isChecked) => raise({ id: "LeftHandedFretboard", isLeftHanded: isChecked }));
    ...
    createCheckbox(
        ...
        "Flip Nut",
        state.isNutFlipped,
        (isChecked) => raise({ id: "FlipNut", isNutFlipped: isChecked }));
