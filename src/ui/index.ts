...
export function arcPath(innerR: number, outerR: number, startAngle: number, endAngle: number, padAngle = 0): string {
    ...
    const x1 = Math.sin(sa) * outerR,  y1 = -Math.cos(sa) * outerR;  // outer start
    const x2 = Math.sin(ea) * outerR,  y2 = -Math.cos(ea) * outerR;  // outer end
    const x3 = Math.sin(ea) * innerR,  y3 = -Math.cos(ea) * innerR;  // inner end
    const x4 = Math.sin(sa) * innerR,  y4 = -Math.cos(sa) * innerR;  // inner start
