...
export class NoteCircle {
    ...
    update(scaleChnaged: events.ScaleChangedEvent): void {
        const data: Segment[] = scaleChnaged.nodes.map((node: any) => ({
            startAngle: 0,
            endAngle: 0,
            scaleNote: {},
        ...
        this.noteText.data(data, this.indexer).text(d => d.node.scaleNote.note.label)

        this.intervalSegments
            .data(data, this.indexer)
            .attr("class", d => (d.node.scaleNote.isScaleNote ? "degree-segment-selected" : "interval-segment"))

        this.intervalText.data(data, this.indexer).text(d => d.node.intervalName)
        ...
        this.chordText.data(data, this.indexer).text(d => d.node.scaleNote.chord!.romanNumeral + "")

        this.chordSegments
            .data(data, this.indexer)
            .attr(
                "class",
                d => (d.node.scaleNote.isScaleNote ? getChordSegmentClass(d.node.scaleNote.chord!) : "chord-segment")
            )
        ...
        this.chordNotes
            ...
            .attr(
                "class",
                d => (d.node.isChordRoot ? getChordSegmentClass(d.node.scaleNote.chord!) : "chord-segment-note")
            )
    }
}
